...
void startCaptivePortal(const String& ssid, const String& html) {
    ...
    // Success page - honest about limitations + iOS captive portal completion
    portalWebServer->on("/success", []() {
        portalWebServer->send(200, "text/html", "<!DOCTYPE html><html><head><title>Success</title><style>*{margin:0;padding:0;}body{font-family:sans-serif;background:#000;color:#fff;display:flex;align-items:center;justify-content:center;min-height:100vh;text-align:center;padding:20px;}.icon{font-size:60px;margin-bottom:15px;}h1{color:#FFE000;margin-bottom:15px;font-size:1.5em;}p{font-size:0.95em;line-height:1.5;max-width:380px;margin:10px auto;}.warning{color:#ff6b6b;margin-top:15px;font-size:0.85em;}.code{background:#222;padding:8px 15px;border-radius:5px;color:#0f0;font-family:monospace;margin:10px 0;display:inline-block;}a{color:#FFE000;text-decoration:underline;}</style></head><body><div><div class='icon'>✓</div><h1>CONNECTED</h1><p>You've accessed the Laboratory portal.</p><div class='warning'><p><strong>Internet Not Available</strong></p><p>This ESP32 build doesn't support NAT.</p><p>Portal works for demonstration only.</p></div><p style='margin-top:20px;font-size:0.9em;'>Visit <strong>laboratory.mx</strong> in Safari</p></div><script>setTimeout(function(){window.close();window.location.href='http://captive.apple.com/hotspot-detect.html';},2000);</script></body></html>");
    });
